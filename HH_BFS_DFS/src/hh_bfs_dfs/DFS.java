...
public class DFS {
    ...
    private int time;
    private ArrayList<javax.swing.Timer> t;
    ...
    public void dfs_recursive(Node node){
        ...
        t.add(new javax.swing.Timer(time, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                cls.setColorOfNode(node.getLoc(), Color.GRAY);
                cls.repaint();
                lb.setText(lb.getText().concat(" "+ node.label));
            }
        }));
        time += 1000;
        
        ArrayList<Node> adjacency = getAdjacencyOfNode(node, matrix);
        for (Node adjOfNode : adjacency) {
            if(adjOfNode.color.equals("WHITE")){
                dfs_recursive(adjOfNode);
            }
        }
        node.color = "BLACK";
        t.add(new javax.swing.Timer(time, new ActionListener() {
